...
def sim(z_i, z_j):
    ...
    norm_dot_product = torch.cosine_similarity(z_i, z_j, dim=-1)
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        a = np.exp(sim(z_k, z_k_N) / tau)
        b1 = 0
        b2 = 0
        for i in range(2*N):
            if i != k:
                b1 += np.exp(sim(z_k, out[i]) / tau)
            if i != k + N:
                b2 += np.exp(sim(z_k_N, out[i]) / tau)
        total_loss += -np.log(a / b1) - np.log(a / b2)
...
def compute_sim_matrix(out):
    ...
    nor = torch.linalg.norm(out, dim=1, keepdim=True)
    sim_matrix = torch.mm(out, out.T) / torch.mm(nor, nor.T)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pairs = sim_pairs.repeat(2, 1)
