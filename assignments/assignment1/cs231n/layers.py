...
def svm_loss(x, y):
    ...
    correct_class_score = np.reshape(correct_class_score, (n_train, 1))
    tLoss = scores + 1 - correct_class_score
    tLoss[tLoss < 0] = 0
    tLoss[np.arange(n_train), y] = 0
    loss = np.sum(tLoss) / n_train
    dx = np.zeros_like(x)
    dx[tLoss > 0] = 1
    r_sum = np.sum(tLoss > 0, axis=1)
    dx[np.arange(n_train), y] = -r_sum
    dx /= n_train
